// quiz_pack6.js
// PuzzlePilot Quiz - Question Pack 6
// 40 additional questions

const questions = [

    // =========================================================
    // MUSIC
    // =========================================================

    {
        id: 'music_0037',
        category: 'Music',
        question:
            'Which band released the 1979 album "The Wall"?',
        answers: [
            'Pink Floyd',
            'Genesis',
            'Yes',
            'The Who'
        ],
        correctAnswer:
            'Pink Floyd',
        difficulty:
            'Easy',
        tags: [
            '1970s',
            'albums',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0038',
        category: 'Music',
        question:
            'Which singer had a 1964 hit with "Downtown"?',
        answers: [
            'Petula Clark',
            'Cilla Black',
            'Sandie Shaw',
            'Dusty Springfield'
        ],
        correctAnswer:
            'Petula Clark',
        difficulty:
            'Medium',
        tags: [
            '1960s',
            'british',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0039',
        category: 'Music',
        question:
            'Which band recorded "Everybody Wants to Rule the World"?',
        answers: [
            'Tears for Fears',
            'Simple Minds',
            'Ultravox',
            'Spandau Ballet'
        ],
        correctAnswer:
            'Tears for Fears',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0040',
        category: 'Music',
        question:
            'Which singer released the 1997 album "Come On Over"?',
        answers: [
            'Shania Twain',
            'Celine Dion',
            'Faith Hill',
            'LeAnn Rimes'
        ],
        correctAnswer:
            'Shania Twain',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'albums',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0041',
        category: 'Music',
        question:
            'Which group had hits with "Stayin\' Alive" and "Night Fever"?',
        answers: [
            'Bee Gees',
            'Earth, Wind & Fire',
            'KC and the Sunshine Band',
            'The Jacksons'
        ],
        correctAnswer:
            'Bee Gees',
        difficulty:
            'Easy',
        tags: [
            '1970s',
            'disco',
            'groups'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0042',
        category: 'Music',
        question:
            'Which singer released "Nothing Compares 2 U" in 1990?',
        answers: [
            'Sinéad O\'Connor',
            'Annie Lennox',
            'Dolores O\'Riordan',
            'Björk'
        ],
        correctAnswer:
            'Sinéad O\'Connor',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0043',
        category: 'Music',
        question:
            'Which band featured Debbie Harry as its lead singer?',
        answers: [
            'Blondie',
            'The Pretenders',
            'Heart',
            'The Bangles'
        ],
        correctAnswer:
            'Blondie',
        difficulty:
            'Easy',
        tags: [
            '1970s',
            'bands',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0044',
        category: 'Music',
        question:
            'Which British singer released the album "No Jacket Required" in 1985?',
        answers: [
            'Phil Collins',
            'Peter Gabriel',
            'Elton John',
            'George Michael'
        ],
        correctAnswer:
            'Phil Collins',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'albums',
            'british'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // TELEVISION
    // =========================================================

    {
        id: 'tv_0037',
        category: 'Television',
        question:
            'In "Only Fools and Horses", what is Del Boy\'s first name?',
        answers: [
            'Derek',
            'David',
            'Douglas',
            'Dennis'
        ],
        correctAnswer:
            'Derek',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'only fools and horses'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0038',
        category: 'Television',
        question:
            'Which actress played Fleabag in the television series of the same name?',
        answers: [
            'Phoebe Waller-Bridge',
            'Olivia Colman',
            'Jodie Comer',
            'Sally Hawkins'
        ],
        correctAnswer:
            'Phoebe Waller-Bridge',
        difficulty:
            'Easy',
        tags: [
            'british',
            'comedy',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0039',
        category: 'Television',
        question:
            'What is the name of the coffee shop regularly visited by the characters in "Friends"?',
        answers: [
            'Central Perk',
            'Coffee Central',
            'Java House',
            'Manhattan Coffee'
        ],
        correctAnswer:
            'Central Perk',
        difficulty:
            'Easy',
        tags: [
            'american',
            'friends',
            'sitcoms'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0040',
        category: 'Television',
        question:
            'Which actor played Detective Inspector Jack Frost in "A Touch of Frost"?',
        answers: [
            'David Jason',
            'John Thaw',
            'David Suchet',
            'Martin Shaw'
        ],
        correctAnswer:
            'David Jason',
        difficulty:
            'Medium',
        tags: [
            'british',
            'crime',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0041',
        category: 'Television',
        question:
            'Which fictional town is the setting of "Stranger Things"?',
        answers: [
            'Hawkins',
            'Sunnydale',
            'Riverdale',
            'Hill Valley'
        ],
        correctAnswer:
            'Hawkins',
        difficulty:
            'Medium',
        tags: [
            'american',
            'drama',
            'stranger things'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0042',
        category: 'Television',
        question:
            'In "Keeping Up Appearances", how does Hyacinth Bucket insist her surname is pronounced?',
        answers: [
            'Bouquet',
            'Bucket',
            'Bookay',
            'Boo-kett'
        ],
        correctAnswer:
            'Bouquet',
        difficulty:
            'Easy',
        tags: [
            'british',
            'sitcoms',
            '1990s'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0043',
        category: 'Television',
        question:
            'Which actor played the title character in the BBC series "Luther"?',
        answers: [
            'Idris Elba',
            'David Oyelowo',
            'Adrian Lester',
            'Chiwetel Ejiofor'
        ],
        correctAnswer:
            'Idris Elba',
        difficulty:
            'Easy',
        tags: [
            'british',
            'crime',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0044',
        category: 'Television',
        question:
            'Which long-running British television series features the fictional village of Emmerdale?',
        answers: [
            'Emmerdale',
            'EastEnders',
            'Coronation Street',
            'Hollyoaks'
        ],
        correctAnswer:
            'Emmerdale',
        difficulty:
            'Easy',
        tags: [
            'british',
            'soaps',
            'television'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FILM
    // =========================================================

    {
        id: 'film_0031',
        category: 'Film',
        question:
            'Which actress played Clarice Starling in "The Silence of the Lambs"?',
        answers: [
            'Jodie Foster',
            'Sigourney Weaver',
            'Michelle Pfeiffer',
            'Susan Sarandon'
        ],
        correctAnswer:
            'Jodie Foster',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'actors',
            'thriller'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0032',
        category: 'Film',
        question:
            'Which 1984 film features a supernatural villain called Gozer?',
        answers: [
            'Ghostbusters',
            'Gremlins',
            'Beetlejuice',
            'Poltergeist'
        ],
        correctAnswer:
            'Ghostbusters',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'films',
            'comedy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0033',
        category: 'Film',
        question:
            'Who directed the 1994 film "Pulp Fiction"?',
        answers: [
            'Quentin Tarantino',
            'Martin Scorsese',
            'David Fincher',
            'Oliver Stone'
        ],
        correctAnswer:
            'Quentin Tarantino',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'directors',
            'films'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0034',
        category: 'Film',
        question:
            'Which actress played the title character in the 2001 film "Lara Croft: Tomb Raider"?',
        answers: [
            'Angelina Jolie',
            'Charlize Theron',
            'Milla Jovovich',
            'Kate Beckinsale'
        ],
        correctAnswer:
            'Angelina Jolie',
        difficulty:
            'Medium',
        tags: [
            '2000s',
            'actors',
            'action'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0035',
        category: 'Film',
        question:
            'Which film features the characters Danny Zuko and Sandy Olsson?',
        answers: [
            'Grease',
            'Footloose',
            'Dirty Dancing',
            'Saturday Night Fever'
        ],
        correctAnswer:
            'Grease',
        difficulty:
            'Easy',
        tags: [
            '1970s',
            'musicals',
            'characters'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0036',
        category: 'Film',
        question:
            'Which actor played the villain Hans Gruber in "Die Hard"?',
        answers: [
            'Alan Rickman',
            'Jeremy Irons',
            'Gary Oldman',
            'Timothy Dalton'
        ],
        correctAnswer:
            'Alan Rickman',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'actors',
            'action'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GENERAL KNOWLEDGE
    // =========================================================

    {
        id: 'general_0016',
        category: 'General Knowledge',
        question:
            'How many letters are there in the Greek alphabet?',
        answers: [
            '24',
            '26',
            '22',
            '20'
        ],
        correctAnswer:
            '24',
        difficulty:
            'Medium',
        tags: [
            'alphabet',
            'numbers',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0017',
        category: 'General Knowledge',
        question:
            'What is the name of the imaginary line dividing Earth into the Northern and Southern Hemispheres?',
        answers: [
            'Equator',
            'Prime Meridian',
            'Tropic of Cancer',
            'International Date Line'
        ],
        correctAnswer:
            'Equator',
        difficulty:
            'Easy',
        tags: [
            'earth',
            'geography',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0018',
        category: 'General Knowledge',
        question:
            'Which board game includes the properties Mayfair and Park Lane in its standard UK edition?',
        answers: [
            'Monopoly',
            'Cluedo',
            'Risk',
            'Scrabble'
        ],
        correctAnswer:
            'Monopoly',
        difficulty:
            'Easy',
        tags: [
            'games',
            'british',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // HISTORY
    // =========================================================

    {
        id: 'history_0015',
        category: 'History',
        question:
            'Which English queen was known as the "Virgin Queen"?',
        answers: [
            'Elizabeth I',
            'Victoria',
            'Mary I',
            'Anne'
        ],
        correctAnswer:
            'Elizabeth I',
        difficulty:
            'Easy',
        tags: [
            'british',
            'monarchy',
            'tudors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0016',
        category: 'History',
        question:
            'Which explorer led the expedition that first reached the South Pole in 1911?',
        answers: [
            'Roald Amundsen',
            'Robert Falcon Scott',
            'Ernest Shackleton',
            'Edmund Hillary'
        ],
        correctAnswer:
            'Roald Amundsen',
        difficulty:
            'Medium',
        tags: [
            'exploration',
            '20th century',
            'antarctica'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0017',
        category: 'History',
        question:
            'Which ancient civilisation used a writing system known as hieroglyphics?',
        answers: [
            'Ancient Egyptians',
            'Vikings',
            'Spartans',
            'Phoenicians'
        ],
        correctAnswer:
            'Ancient Egyptians',
        difficulty:
            'Easy',
        tags: [
            'ancient history',
            'egypt',
            'writing'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GEOGRAPHY
    // =========================================================

    {
        id: 'geography_0015',
        category: 'Geography',
        question:
            'Which European capital stands on the River Liffey?',
        answers: [
            'Dublin',
            'Vienna',
            'Prague',
            'Budapest'
        ],
        correctAnswer:
            'Dublin',
        difficulty:
            'Medium',
        tags: [
            'europe',
            'capitals',
            'rivers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0016',
        category: 'Geography',
        question:
            'Which is the largest country in South America by area?',
        answers: [
            'Brazil',
            'Argentina',
            'Peru',
            'Colombia'
        ],
        correctAnswer:
            'Brazil',
        difficulty:
            'Easy',
        tags: [
            'south america',
            'countries',
            'geography'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0017',
        category: 'Geography',
        question:
            'Mount Fuji is located on which Japanese island?',
        answers: [
            'Honshu',
            'Hokkaido',
            'Kyushu',
            'Shikoku'
        ],
        correctAnswer:
            'Honshu',
        difficulty:
            'Medium',
        tags: [
            'japan',
            'mountains',
            'islands'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SCIENCE
    // =========================================================

    {
        id: 'science_0014',
        category: 'Science',
        question:
            'What is the chemical symbol for potassium?',
        answers: [
            'K',
            'P',
            'Po',
            'Pt'
        ],
        correctAnswer:
            'K',
        difficulty:
            'Medium',
        tags: [
            'chemistry',
            'elements',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0015',
        category: 'Science',
        question:
            'Which part of a plant usually carries out most photosynthesis?',
        answers: [
            'Leaves',
            'Roots',
            'Flowers',
            'Seeds'
        ],
        correctAnswer:
            'Leaves',
        difficulty:
            'Easy',
        tags: [
            'biology',
            'plants',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0016',
        category: 'Science',
        question:
            'Which blood type is commonly described as the universal donor for red blood cells?',
        answers: [
            'O negative',
            'AB positive',
            'A positive',
            'B negative'
        ],
        correctAnswer:
            'O negative',
        difficulty:
            'Medium',
        tags: [
            'biology',
            'blood',
            'science'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FOOD AND DRINK
    // =========================================================

    {
        id: 'food_0010',
        category: 'Food and Drink',
        question:
            'Which nut is traditionally used to make marzipan?',
        answers: [
            'Almond',
            'Hazelnut',
            'Walnut',
            'Pistachio'
        ],
        correctAnswer:
            'Almond',
        difficulty:
            'Easy',
        tags: [
            'baking',
            'ingredients',
            'food'
        ],
        dailyEligible:
            true
    },

    {
        id: 'food_0011',
        category: 'Food and Drink',
        question:
            'Which cheese is traditionally used in a Greek salad?',
        answers: [
            'Feta',
            'Brie',
            'Gouda',
            'Stilton'
        ],
        correctAnswer:
            'Feta',
        difficulty:
            'Easy',
        tags: [
            'cheese',
            'greek',
            'food'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SPORT
    // =========================================================

    {
        id: 'sport_0010',
        category: 'Sport',
        question:
            'How many points is a try worth in rugby union?',
        answers: [
            '5',
            '3',
            '4',
            '6'
        ],
        correctAnswer:
            '5',
        difficulty:
            'Easy',
        tags: [
            'rugby union',
            'rules',
            'sport'
        ],
        dailyEligible:
            true
    },

    {
        id: 'sport_0011',
        category: 'Sport',
        question:
            'Which country won the FIFA World Cup in 1966?',
        answers: [
            'England',
            'West Germany',
            'Brazil',
            'Italy'
        ],
        correctAnswer:
            'England',
        difficulty:
            'Easy',
        tags: [
            'football',
            'world cup',
            '1960s'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // LITERATURE
    // =========================================================

    {
        id: 'literature_0008',
        category: 'Literature',
        question:
            'Which author created the detective Miss Marple?',
        answers: [
            'Agatha Christie',
            'Dorothy L. Sayers',
            'P. D. James',
            'Ruth Rendell'
        ],
        correctAnswer:
            'Agatha Christie',
        difficulty:
            'Easy',
        tags: [
            'british',
            'crime',
            'authors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'literature_0009',
        category: 'Literature',
        question:
            'Which novel by George Orwell features the character Winston Smith?',
        answers: [
            'Nineteen Eighty-Four',
            'Animal Farm',
            'Keep the Aspidistra Flying',
            'Coming Up for Air'
        ],
        correctAnswer:
            'Nineteen Eighty-Four',
        difficulty:
            'Medium',
        tags: [
            'british',
            'novels',
            'george orwell'
        ],
        dailyEligible:
            true
    }

];

module.exports = questions;
