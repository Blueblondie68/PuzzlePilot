// quiz_pack11.js
// PuzzlePilot Quiz - Question Pack 11
// 40 additional questions

const questions = [

    // =========================================================
    // MUSIC
    // =========================================================

    {
        id: 'music_0077',
        category: 'Music',
        question:
            'Which band released the 1977 album "Out of the Blue"?',
        answers: [
            'Electric Light Orchestra',
            'Supertramp',
            '10cc',
            'Genesis'
        ],
        correctAnswer:
            'Electric Light Orchestra',
        difficulty:
            'Medium',
        tags: [
            '1970s',
            'albums',
            'british'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0078',
        category: 'Music',
        question:
            'Which singer had a 1988 hit with "Fast Car"?',
        answers: [
            'Tracy Chapman',
            'Suzanne Vega',
            'Tina Turner',
            'Sinéad O\'Connor'
        ],
        correctAnswer:
            'Tracy Chapman',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0079',
        category: 'Music',
        question:
            'Which British band released the song "Common People" in 1995?',
        answers: [
            'Pulp',
            'Blur',
            'Oasis',
            'Suede'
        ],
        correctAnswer:
            'Pulp',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'britpop',
            'british'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0080',
        category: 'Music',
        question:
            'Which singer had a 1986 hit with "The Lady in Red"?',
        answers: [
            'Chris de Burgh',
            'Paul Young',
            'Bryan Ferry',
            'Peter Cetera'
        ],
        correctAnswer:
            'Chris de Burgh',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0081',
        category: 'Music',
        question:
            'Which band recorded the 1983 hit "Blue Monday"?',
        answers: [
            'New Order',
            'Depeche Mode',
            'The Cure',
            'Joy Division'
        ],
        correctAnswer:
            'New Order',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0082',
        category: 'Music',
        question:
            'Which singer released the 1997 hit "Torn"?',
        answers: [
            'Natalie Imbruglia',
            'Sheryl Crow',
            'Alanis Morissette',
            'Meredith Brooks'
        ],
        correctAnswer:
            'Natalie Imbruglia',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0083',
        category: 'Music',
        question:
            'Which group had a 1990 hit with "Groove Is in the Heart"?',
        answers: [
            'Deee-Lite',
            'En Vogue',
            'Technotronic',
            'Snap!'
        ],
        correctAnswer:
            'Deee-Lite',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'dance',
            'groups'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0084',
        category: 'Music',
        question:
            'Which singer-songwriter released the hit "Babylon" from the album "White Ladder"?',
        answers: [
            'David Gray',
            'James Blunt',
            'Damien Rice',
            'David Byrne'
        ],
        correctAnswer:
            'David Gray',
        difficulty:
            'Medium',
        tags: [
            '2000s',
            'british',
            'singer-songwriter'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // TELEVISION
    // =========================================================

    {
        id: 'tv_0077',
        category: 'Television',
        question:
            'In "Line of Duty", which anti-corruption unit is central to the series?',
        answers: [
            'AC-12',
            'CID-7',
            'OCU-4',
            'MIT-10'
        ],
        correctAnswer:
            'AC-12',
        difficulty:
            'Easy',
        tags: [
            'british',
            'crime',
            'line of duty'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0078',
        category: 'Television',
        question:
            'Which actor played Father Jack Hackett in "Father Ted"?',
        answers: [
            'Frank Kelly',
            'Dermot Morgan',
            'Ardal O\'Hanlon',
            'Pauline McLynn'
        ],
        correctAnswer:
            'Frank Kelly',
        difficulty:
            'Medium',
        tags: [
            'british',
            'irish',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0079',
        category: 'Television',
        question:
            'In "Gavin & Stacey", in which Welsh town does Stacey live?',
        answers: [
            'Barry',
            'Swansea',
            'Newport',
            'Bridgend'
        ],
        correctAnswer:
            'Barry',
        difficulty:
            'Easy',
        tags: [
            'british',
            'sitcoms',
            'gavin and stacey'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0080',
        category: 'Television',
        question:
            'Which actor played Malcolm Tucker in "The Thick of It"?',
        answers: [
            'Peter Capaldi',
            'Chris Addison',
            'Roger Allam',
            'Steve Coogan'
        ],
        correctAnswer:
            'Peter Capaldi',
        difficulty:
            'Medium',
        tags: [
            'british',
            'comedy',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0081',
        category: 'Television',
        question:
            'Which actress played Fleabag\'s sister Claire in "Fleabag"?',
        answers: [
            'Sian Clifford',
            'Olivia Colman',
            'Phoebe Waller-Bridge',
            'Fiona Shaw'
        ],
        correctAnswer:
            'Sian Clifford',
        difficulty:
            'Medium',
        tags: [
            'british',
            'comedy',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0082',
        category: 'Television',
        question:
            'Which actress played the title character in "Miranda"?',
        answers: [
            'Miranda Hart',
            'Sally Phillips',
            'Sarah Millican',
            'Catherine Tate'
        ],
        correctAnswer:
            'Miranda Hart',
        difficulty:
            'Easy',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0083',
        category: 'Television',
        question:
            'In "Only Fools and Horses", what is Trigger\'s real first name?',
        answers: [
            'Colin',
            'Roger',
            'Trevor',
            'Reginald'
        ],
        correctAnswer:
            'Colin',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'only fools and horses'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0084',
        category: 'Television',
        question:
            'Which drama series stars Cillian Murphy as Thomas Shelby?',
        answers: [
            'Peaky Blinders',
            'Boardwalk Empire',
            'Taboo',
            'Ripper Street'
        ],
        correctAnswer:
            'Peaky Blinders',
        difficulty:
            'Easy',
        tags: [
            'british',
            'drama',
            'characters'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FILM
    // =========================================================

    {
        id: 'film_0061',
        category: 'Film',
        question:
            'Which actress played Bridget Gregory in the 1994 film "The Last Seduction"?',
        answers: [
            'Linda Fiorentino',
            'Sharon Stone',
            'Demi Moore',
            'Michelle Pfeiffer'
        ],
        correctAnswer:
            'Linda Fiorentino',
        difficulty:
            'Hard',
        tags: [
            '1990s',
            'actors',
            'thriller'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0062',
        category: 'Film',
        question:
            'Which actor played Vincent Vega in "Pulp Fiction"?',
        answers: [
            'John Travolta',
            'Samuel L. Jackson',
            'Bruce Willis',
            'Tim Roth'
        ],
        correctAnswer:
            'John Travolta',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'actors',
            'crime'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0063',
        category: 'Film',
        question:
            'Which actor played Dr Emmett Brown in the "Back to the Future" films?',
        answers: [
            'Christopher Lloyd',
            'Michael J. Fox',
            'Thomas F. Wilson',
            'Crispin Glover'
        ],
        correctAnswer:
            'Christopher Lloyd',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'actors',
            'science fiction'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0064',
        category: 'Film',
        question:
            'Which actor played Sam Wheat in the 1990 film "Ghost"?',
        answers: [
            'Patrick Swayze',
            'Kevin Costner',
            'Richard Gere',
            'Tom Cruise'
        ],
        correctAnswer:
            'Patrick Swayze',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'actors',
            'romance'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0065',
        category: 'Film',
        question:
            'Which actor played Axel Foley in the "Beverly Hills Cop" films?',
        answers: [
            'Eddie Murphy',
            'Richard Pryor',
            'Dan Aykroyd',
            'Bill Murray'
        ],
        correctAnswer:
            'Eddie Murphy',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'actors',
            'comedy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0066',
        category: 'Film',
        question:
            'Which actress played Elle Woods in "Legally Blonde"?',
        answers: [
            'Reese Witherspoon',
            'Cameron Diaz',
            'Jennifer Aniston',
            'Kate Hudson'
        ],
        correctAnswer:
            'Reese Witherspoon',
        difficulty:
            'Easy',
        tags: [
            '2000s',
            'actors',
            'comedy'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GENERAL KNOWLEDGE
    // =========================================================

    {
        id: 'general_0031',
        category: 'General Knowledge',
        question:
            'How many days are there in a leap year?',
        answers: [
            '366',
            '365',
            '364',
            '367'
        ],
        correctAnswer:
            '366',
        difficulty:
            'Easy',
        tags: [
            'calendar',
            'numbers',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0032',
        category: 'General Knowledge',
        question:
            'What is the collective name for a group of lions?',
        answers: [
            'Pride',
            'Pack',
            'Herd',
            'Flock'
        ],
        correctAnswer:
            'Pride',
        difficulty:
            'Easy',
        tags: [
            'animals',
            'words',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0033',
        category: 'General Knowledge',
        question:
            'How many colours are traditionally listed in a rainbow?',
        answers: [
            '7',
            '6',
            '8',
            '9'
        ],
        correctAnswer:
            '7',
        difficulty:
            'Easy',
        tags: [
            'nature',
            'numbers',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // HISTORY
    // =========================================================

    {
        id: 'history_0030',
        category: 'History',
        question:
            'Which English king was killed at the Battle of Hastings in 1066?',
        answers: [
            'Harold II',
            'William I',
            'Edward the Confessor',
            'Henry I'
        ],
        correctAnswer:
            'Harold II',
        difficulty:
            'Medium',
        tags: [
            'british',
            'medieval',
            'monarchy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0031',
        category: 'History',
        question:
            'Which English king was nicknamed "Longshanks"?',
        answers: [
            'Edward I',
            'Edward II',
            'Henry III',
            'Richard I'
        ],
        correctAnswer:
            'Edward I',
        difficulty:
            'Medium',
        tags: [
            'british',
            'medieval',
            'monarchy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0032',
        category: 'History',
        question:
            'Which Tudor monarch was the daughter of Henry VIII and Anne Boleyn?',
        answers: [
            'Elizabeth I',
            'Mary I',
            'Lady Jane Grey',
            'Mary, Queen of Scots'
        ],
        correctAnswer:
            'Elizabeth I',
        difficulty:
            'Easy',
        tags: [
            'british',
            'tudors',
            'monarchy'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GEOGRAPHY
    // =========================================================

    {
        id: 'geography_0030',
        category: 'Geography',
        question:
            'What is the capital city of Iceland?',
        answers: [
            'Reykjavík',
            'Oslo',
            'Helsinki',
            'Copenhagen'
        ],
        correctAnswer:
            'Reykjavík',
        difficulty:
            'Easy',
        tags: [
            'europe',
            'capitals',
            'iceland'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0031',
        category: 'Geography',
        question:
            'Which mountain is the highest in Wales?',
        answers: [
            'Yr Wyddfa (Snowdon)',
            'Pen y Fan',
            'Cadair Idris',
            'Tryfan'
        ],
        correctAnswer:
            'Yr Wyddfa (Snowdon)',
        difficulty:
            'Medium',
        tags: [
            'british',
            'wales',
            'mountains'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0032',
        category: 'Geography',
        question:
            'Which country has the city of Salzburg?',
        answers: [
            'Austria',
            'Germany',
            'Switzerland',
            'Belgium'
        ],
        correctAnswer:
            'Austria',
        difficulty:
            'Easy',
        tags: [
            'europe',
            'cities',
            'countries'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SCIENCE
    // =========================================================

    {
        id: 'science_0029',
        category: 'Science',
        question:
            'Which blood component helps the blood to clot?',
        answers: [
            'Platelets',
            'Red blood cells',
            'White blood cells',
            'Plasma'
        ],
        correctAnswer:
            'Platelets',
        difficulty:
            'Medium',
        tags: [
            'biology',
            'human body',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0030',
        category: 'Science',
        question:
            'Which metal is liquid at room temperature?',
        answers: [
            'Mercury',
            'Iron',
            'Copper',
            'Aluminium'
        ],
        correctAnswer:
            'Mercury',
        difficulty:
            'Easy',
        tags: [
            'chemistry',
            'elements',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0031',
        category: 'Science',
        question:
            'Which part of a plant usually absorbs water from the soil?',
        answers: [
            'Roots',
            'Leaves',
            'Flowers',
            'Fruit'
        ],
        correctAnswer:
            'Roots',
        difficulty:
            'Easy',
        tags: [
            'biology',
            'plants',
            'science'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FOOD AND DRINK
    // =========================================================

    {
        id: 'food_0020',
        category: 'Food and Drink',
        question:
            'Which nuts are traditionally used to make marzipan?',
        answers: [
            'Almonds',
            'Hazelnuts',
            'Walnuts',
            'Pistachios'
        ],
        correctAnswer:
            'Almonds',
        difficulty:
            'Easy',
        tags: [
            'ingredients',
            'sweets',
            'food'
        ],
        dailyEligible:
            true
    },

    {
        id: 'food_0021',
        category: 'Food and Drink',
        question:
            'Which fruit is the main ingredient in a traditional tarte Tatin?',
        answers: [
            'Apple',
            'Pear',
            'Plum',
            'Peach'
        ],
        correctAnswer:
            'Apple',
        difficulty:
            'Easy',
        tags: [
            'fruit',
            'desserts',
            'food'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SPORT
    // =========================================================

    {
        id: 'sport_0020',
        category: 'Sport',
        question:
            'How many players from one team are on the court at a time in volleyball?',
        answers: [
            '6',
            '5',
            '7',
            '8'
        ],
        correctAnswer:
            '6',
        difficulty:
            'Easy',
        tags: [
            'volleyball',
            'players',
            'sport'
        ],
        dailyEligible:
            true
    },

    {
        id: 'sport_0021',
        category: 'Sport',
        question:
            'Which sport uses the terms love, deuce and ace?',
        answers: [
            'Tennis',
            'Badminton',
            'Squash',
            'Table tennis'
        ],
        correctAnswer:
            'Tennis',
        difficulty:
            'Easy',
        tags: [
            'tennis',
            'terminology',
            'sport'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // LITERATURE
    // =========================================================

    {
        id: 'literature_0018',
        category: 'Literature',
        question:
            'Who wrote the novel "The Remains of the Day"?',
        answers: [
            'Kazuo Ishiguro',
            'Ian McEwan',
            'Julian Barnes',
            'Martin Amis'
        ],
        correctAnswer:
            'Kazuo Ishiguro',
        difficulty:
            'Medium',
        tags: [
            'british',
            'authors',
            'novels'
        ],
        dailyEligible:
            true
    },

    {
        id: 'literature_0019',
        category: 'Literature',
        question:
            'Which author created the character Paddington Bear?',
        answers: [
            'Michael Bond',
            'A. A. Milne',
            'Kenneth Grahame',
            'Roald Dahl'
        ],
        correctAnswer:
            'Michael Bond',
        difficulty:
            'Medium',
        tags: [
            'british',
            'authors',
            'children'
        ],
        dailyEligible:
            true
    }

];

module.exports = questions;
