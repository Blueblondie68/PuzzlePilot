// quiz_pack12.js
// PuzzlePilot Quiz - Question Pack 12
// 40 additional questions

const questions = [

    // =========================================================
    // MUSIC
    // =========================================================

    {
        id: 'music_0085',
        category: 'Music',
        question:
            'Which singer had a 1985 hit with "Running Up That Hill"?',
        answers: [
            'Kate Bush',
            'Alison Moyet',
            'Annie Lennox',
            'Kim Wilde'
        ],
        correctAnswer:
            'Kate Bush',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0086',
        category: 'Music',
        question:
            'Which singer released the 1993 hit "Dreams"?',
        answers: [
            'Gabrielle',
            'Des\'ree',
            'Lisa Stansfield',
            'Beverley Knight'
        ],
        correctAnswer:
            'Gabrielle',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'british',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0087',
        category: 'Music',
        question:
            'Which band had a 1979 hit with "Message in a Bottle"?',
        answers: [
            'The Police',
            'The Jam',
            'The Clash',
            'Squeeze'
        ],
        correctAnswer:
            'The Police',
        difficulty:
            'Medium',
        tags: [
            '1970s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0088',
        category: 'Music',
        question:
            'Which singer released the 1994 hit "Stay (I Missed You)"?',
        answers: [
            'Lisa Loeb',
            'Sheryl Crow',
            'Alanis Morissette',
            'Suzanne Vega'
        ],
        correctAnswer:
            'Lisa Loeb',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0089',
        category: 'Music',
        question:
            'Which British group released "West End Girls"?',
        answers: [
            'Pet Shop Boys',
            'Erasure',
            'Soft Cell',
            'Orchestral Manoeuvres in the Dark'
        ],
        correctAnswer:
            'Pet Shop Boys',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'duos'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0090',
        category: 'Music',
        question:
            'Which singer had a 1964 hit with "My Boy Lollipop"?',
        answers: [
            'Millie Small',
            'Sandie Shaw',
            'Cilla Black',
            'Lulu'
        ],
        correctAnswer:
            'Millie Small',
        difficulty:
            'Medium',
        tags: [
            '1960s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0091',
        category: 'Music',
        question:
            'Which band released the 2004 album "Hot Fuss"?',
        answers: [
            'The Killers',
            'Kings of Leon',
            'Franz Ferdinand',
            'The Strokes'
        ],
        correctAnswer:
            'The Killers',
        difficulty:
            'Medium',
        tags: [
            '2000s',
            'albums',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0092',
        category: 'Music',
        question:
            'Which singer had a hit with "You\'re So Vain" in the 1970s?',
        answers: [
            'Carly Simon',
            'Carole King',
            'Joni Mitchell',
            'Linda Ronstadt'
        ],
        correctAnswer:
            'Carly Simon',
        difficulty:
            'Medium',
        tags: [
            '1970s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // TELEVISION
    // =========================================================

    {
        id: 'tv_0085',
        category: 'Television',
        question:
            'Which actor played Jim Royle in "The Royle Family"?',
        answers: [
            'Ricky Tomlinson',
            'Craig Cash',
            'Geoffrey Hughes',
            'John Thomson'
        ],
        correctAnswer:
            'Ricky Tomlinson',
        difficulty:
            'Easy',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0086',
        category: 'Television',
        question:
            'In "The Big Bang Theory", what is Sheldon Cooper\'s profession?',
        answers: [
            'Physicist',
            'Chemist',
            'Engineer',
            'Biologist'
        ],
        correctAnswer:
            'Physicist',
        difficulty:
            'Easy',
        tags: [
            'american',
            'sitcoms',
            'characters'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0087',
        category: 'Television',
        question:
            'Which actress played Sybil Fawlty in "Fawlty Towers"?',
        answers: [
            'Prunella Scales',
            'Penelope Keith',
            'Wendy Craig',
            'Felicity Kendal'
        ],
        correctAnswer:
            'Prunella Scales',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0088',
        category: 'Television',
        question:
            'Which television detective is assisted by Sergeant Lewis in the original series?',
        answers: [
            'Inspector Morse',
            'Inspector Frost',
            'Inspector Wexford',
            'Inspector Barnaby'
        ],
        correctAnswer:
            'Inspector Morse',
        difficulty:
            'Medium',
        tags: [
            'british',
            'crime',
            'characters'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0089',
        category: 'Television',
        question:
            'Which US sitcom features the characters Jay, Gloria, Claire, Phil and Mitchell?',
        answers: [
            'Modern Family',
            'Arrested Development',
            'Everybody Loves Raymond',
            'The Middle'
        ],
        correctAnswer:
            'Modern Family',
        difficulty:
            'Easy',
        tags: [
            'american',
            'sitcoms',
            'characters'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0090',
        category: 'Television',
        question:
            'Which actor played Detective Chief Inspector Tom Barnaby in "Midsomer Murders"?',
        answers: [
            'John Nettles',
            'David Jason',
            'John Thaw',
            'Kevin Whately'
        ],
        correctAnswer:
            'John Nettles',
        difficulty:
            'Medium',
        tags: [
            'british',
            'crime',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0091',
        category: 'Television',
        question:
            'In "Derry Girls", the main characters attend school in which part of the UK?',
        answers: [
            'Northern Ireland',
            'Scotland',
            'Wales',
            'England'
        ],
        correctAnswer:
            'Northern Ireland',
        difficulty:
            'Easy',
        tags: [
            'british',
            'comedy',
            'derry girls'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0092',
        category: 'Television',
        question:
            'Which actress played Ruth Jones\'s character Nessa\'s best friend Stacey in "Gavin & Stacey"?',
        answers: [
            'Joanna Page',
            'Sheridan Smith',
            'Sally Lindsay',
            'Katherine Parkinson'
        ],
        correctAnswer:
            'Joanna Page',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FILM
    // =========================================================

    {
        id: 'film_0067',
        category: 'Film',
        question:
            'Which actor played the title character in the 1990 film "Cyrano de Bergerac"?',
        answers: [
            'Gérard Depardieu',
            'Jean Reno',
            'Daniel Auteuil',
            'Vincent Cassel'
        ],
        correctAnswer:
            'Gérard Depardieu',
        difficulty:
            'Hard',
        tags: [
            '1990s',
            'actors',
            'international film'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0068',
        category: 'Film',
        question:
            'Which film features a young lion named Simba?',
        answers: [
            'The Lion King',
            'Madagascar',
            'The Jungle Book',
            'Tarzan'
        ],
        correctAnswer:
            'The Lion King',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'animation',
            'characters'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0069',
        category: 'Film',
        question:
            'Which actress played Katniss Everdeen in "The Hunger Games" films?',
        answers: [
            'Jennifer Lawrence',
            'Emma Stone',
            'Shailene Woodley',
            'Kristen Stewart'
        ],
        correctAnswer:
            'Jennifer Lawrence',
        difficulty:
            'Easy',
        tags: [
            '2010s',
            'actors',
            'film series'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0070',
        category: 'Film',
        question:
            'Which 1988 film stars Bruce Willis as police officer John McClane?',
        answers: [
            'Die Hard',
            'Lethal Weapon',
            'Predator',
            'RoboCop'
        ],
        correctAnswer:
            'Die Hard',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'action',
            'characters'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0071',
        category: 'Film',
        question:
            'Which actress played Mary Poppins in the 1964 film?',
        answers: [
            'Julie Andrews',
            'Audrey Hepburn',
            'Debbie Reynolds',
            'Judy Garland'
        ],
        correctAnswer:
            'Julie Andrews',
        difficulty:
            'Easy',
        tags: [
            '1960s',
            'actors',
            'musicals'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0072',
        category: 'Film',
        question:
            'Which actor played Ferris Bueller in the 1986 film "Ferris Bueller\'s Day Off"?',
        answers: [
            'Matthew Broderick',
            'Michael J. Fox',
            'John Cusack',
            'Emilio Estevez'
        ],
        correctAnswer:
            'Matthew Broderick',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'comedy',
            'actors'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GENERAL KNOWLEDGE
    // =========================================================

    {
        id: 'general_0034',
        category: 'General Knowledge',
        question:
            'How many faces does a standard tetrahedron have?',
        answers: [
            '4',
            '6',
            '8',
            '12'
        ],
        correctAnswer:
            '4',
        difficulty:
            'Medium',
        tags: [
            'numbers',
            'shapes',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0035',
        category: 'General Knowledge',
        question:
            'What is the collective name for a group of crows?',
        answers: [
            'Murder',
            'Pride',
            'Parliament',
            'Pack'
        ],
        correctAnswer:
            'Murder',
        difficulty:
            'Medium',
        tags: [
            'animals',
            'words',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0036',
        category: 'General Knowledge',
        question:
            'Which month gains an extra day during a leap year?',
        answers: [
            'February',
            'January',
            'March',
            'April'
        ],
        correctAnswer:
            'February',
        difficulty:
            'Easy',
        tags: [
            'calendar',
            'dates',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // HISTORY
    // =========================================================

    {
        id: 'history_0033',
        category: 'History',
        question:
            'Which British prime minister was known as the Iron Lady?',
        answers: [
            'Margaret Thatcher',
            'Theresa May',
            'Barbara Castle',
            'Harriet Harman'
        ],
        correctAnswer:
            'Margaret Thatcher',
        difficulty:
            'Easy',
        tags: [
            'british',
            '20th century',
            'politics'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0034',
        category: 'History',
        question:
            'Which ancient Egyptian queen was associated with both Julius Caesar and Mark Antony?',
        answers: [
            'Cleopatra',
            'Nefertiti',
            'Hatshepsut',
            'Nefertari'
        ],
        correctAnswer:
            'Cleopatra',
        difficulty:
            'Easy',
        tags: [
            'ancient history',
            'egypt',
            'royalty'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0035',
        category: 'History',
        question:
            'Which English king signed the death warrant of Mary, Queen of Scots?',
        answers: [
            'Elizabeth I',
            'Henry VIII',
            'James I',
            'Edward VI'
        ],
        correctAnswer:
            'Elizabeth I',
        difficulty:
            'Medium',
        tags: [
            'british',
            'tudors',
            'monarchy'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GEOGRAPHY
    // =========================================================

    {
        id: 'geography_0033',
        category: 'Geography',
        question:
            'What is the capital city of Ireland?',
        answers: [
            'Dublin',
            'Cork',
            'Galway',
            'Limerick'
        ],
        correctAnswer:
            'Dublin',
        difficulty:
            'Easy',
        tags: [
            'ireland',
            'capitals',
            'europe'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0034',
        category: 'Geography',
        question:
            'Which mountain range separates much of France from Spain?',
        answers: [
            'Pyrenees',
            'Alps',
            'Carpathians',
            'Apennines'
        ],
        correctAnswer:
            'Pyrenees',
        difficulty:
            'Medium',
        tags: [
            'europe',
            'mountains',
            'geography'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0035',
        category: 'Geography',
        question:
            'Which US state is made up of a chain of islands in the Pacific Ocean?',
        answers: [
            'Hawaii',
            'Alaska',
            'California',
            'Florida'
        ],
        correctAnswer:
            'Hawaii',
        difficulty:
            'Easy',
        tags: [
            'usa',
            'islands',
            'geography'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SCIENCE
    // =========================================================

    {
        id: 'science_0032',
        category: 'Science',
        question:
            'Which planet is known for the Great Red Spot?',
        answers: [
            'Jupiter',
            'Saturn',
            'Mars',
            'Neptune'
        ],
        correctAnswer:
            'Jupiter',
        difficulty:
            'Easy',
        tags: [
            'space',
            'planets',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0033',
        category: 'Science',
        question:
            'What is the name of the process by which a gas changes into a liquid?',
        answers: [
            'Condensation',
            'Evaporation',
            'Sublimation',
            'Melting'
        ],
        correctAnswer:
            'Condensation',
        difficulty:
            'Easy',
        tags: [
            'physics',
            'states of matter',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0034',
        category: 'Science',
        question:
            'Which vitamin is produced in the skin in response to sunlight?',
        answers: [
            'Vitamin D',
            'Vitamin A',
            'Vitamin C',
            'Vitamin K'
        ],
        correctAnswer:
            'Vitamin D',
        difficulty:
            'Easy',
        tags: [
            'biology',
            'human body',
            'science'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FOOD AND DRINK
    // =========================================================

    {
        id: 'food_0022',
        category: 'Food and Drink',
        question:
            'Which spice is traditionally used to give paella much of its distinctive colour and flavour?',
        answers: [
            'Saffron',
            'Cinnamon',
            'Nutmeg',
            'Cloves'
        ],
        correctAnswer:
            'Saffron',
        difficulty:
            'Medium',
        tags: [
            'spanish',
            'spices',
            'food'
        ],
        dailyEligible:
            true
    },

    {
        id: 'food_0023',
        category: 'Food and Drink',
        question:
            'Which Italian cheese is traditionally used in a classic carbonara?',
        answers: [
            'Pecorino Romano',
            'Gorgonzola',
            'Mascarpone',
            'Mozzarella'
        ],
        correctAnswer:
            'Pecorino Romano',
        difficulty:
            'Medium',
        tags: [
            'italian',
            'cheese',
            'food'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SPORT
    // =========================================================

    {
        id: 'sport_0022',
        category: 'Sport',
        question:
            'In which sport would you perform a slam dunk?',
        answers: [
            'Basketball',
            'Volleyball',
            'Netball',
            'Handball'
        ],
        correctAnswer:
            'Basketball',
        difficulty:
            'Easy',
        tags: [
            'basketball',
            'terminology',
            'sport'
        ],
        dailyEligible:
            true
    },

    {
        id: 'sport_0023',
        category: 'Sport',
        question:
            'How many players from one team are on court at a time in basketball?',
        answers: [
            '5',
            '6',
            '7',
            '8'
        ],
        correctAnswer:
            '5',
        difficulty:
            'Easy',
        tags: [
            'basketball',
            'rules',
            'sport'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // LITERATURE
    // =========================================================

    {
        id: 'literature_0020',
        category: 'Literature',
        question:
            'Who wrote "The Lion, the Witch and the Wardrobe"?',
        answers: [
            'C. S. Lewis',
            'J. R. R. Tolkien',
            'Roald Dahl',
            'Philip Pullman'
        ],
        correctAnswer:
            'C. S. Lewis',
        difficulty:
            'Easy',
        tags: [
            'british',
            'authors',
            'fantasy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'literature_0021',
        category: 'Literature',
        question:
            'Who wrote the novel "The Woman in White"?',
        answers: [
            'Wilkie Collins',
            'Thomas Hardy',
            'Charles Dickens',
            'Anthony Trollope'
        ],
        correctAnswer:
            'Wilkie Collins',
        difficulty:
            'Medium',
        tags: [
            'british',
            'authors',
            'classics'
        ],
        dailyEligible:
            true
    }

];

module.exports = questions;
