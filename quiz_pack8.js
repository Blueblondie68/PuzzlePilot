// quiz_pack8.js
// PuzzlePilot Quiz - Question Pack 8
// 40 additional questions

const questions = [

    // =========================================================
    // MUSIC
    // =========================================================

    {
        id: 'music_0053',
        category: 'Music',
        question:
            'Which band released the 1979 album "Breakfast in America"?',
        answers: [
            'Supertramp',
            'Genesis',
            'Electric Light Orchestra',
            '10cc'
        ],
        correctAnswer:
            'Supertramp',
        difficulty:
            'Medium',
        tags: [
            '1970s',
            'albums',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0054',
        category: 'Music',
        question:
            'Which singer had a 1987 hit with "Heaven Is a Place on Earth"?',
        answers: [
            'Belinda Carlisle',
            'Kim Wilde',
            'Cyndi Lauper',
            'Tiffany'
        ],
        correctAnswer:
            'Belinda Carlisle',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0055',
        category: 'Music',
        question:
            'Which singer had a 1981 hit with "Kids in America"?',
        answers: [
            'Kim Wilde',
            'Toyah Willcox',
            'Hazel O\'Connor',
            'Sheena Easton'
        ],
        correctAnswer:
            'Kim Wilde',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0056',
        category: 'Music',
        question:
            'Which singer released the 1998 hit "...Baby One More Time"?',
        answers: [
            'Britney Spears',
            'Christina Aguilera',
            'Jessica Simpson',
            'Mandy Moore'
        ],
        correctAnswer:
            'Britney Spears',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'pop',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0057',
        category: 'Music',
        question:
            'Which band featured Morrissey as its lead singer?',
        answers: [
            'The Smiths',
            'The Cure',
            'Joy Division',
            'Echo & the Bunnymen'
        ],
        correctAnswer:
            'The Smiths',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0058',
        category: 'Music',
        question:
            'Which singer released the 1971 album "Tapestry"?',
        answers: [
            'Carole King',
            'Joni Mitchell',
            'Carly Simon',
            'Joan Baez'
        ],
        correctAnswer:
            'Carole King',
        difficulty:
            'Medium',
        tags: [
            '1970s',
            'albums',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0059',
        category: 'Music',
        question:
            'Which British group released the 1996 single "Wannabe"?',
        answers: [
            'Spice Girls',
            'All Saints',
            'Eternal',
            'Bananarama'
        ],
        correctAnswer:
            'Spice Girls',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'british',
            'groups'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0060',
        category: 'Music',
        question:
            'Which singer released the album "Graceland" in 1986?',
        answers: [
            'Paul Simon',
            'Billy Joel',
            'Bruce Springsteen',
            'Don Henley'
        ],
        correctAnswer:
            'Paul Simon',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'albums',
            'singers'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // TELEVISION
    // =========================================================

    {
        id: 'tv_0053',
        category: 'TV',
        question:
            'Which actress played Edina Monsoon in "Absolutely Fabulous"?',
        answers: [
            'Jennifer Saunders',
            'Dawn French',
            'Joanna Lumley',
            'Julia Sawalha'
        ],
        correctAnswer:
            'Jennifer Saunders',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0054',
        category: 'TV',
        question:
            'In "Friends", what is the first name of Ross Geller\'s sister?',
        answers: [
            'Monica',
            'Rachel',
            'Phoebe',
            'Janice'
        ],
        correctAnswer:
            'Monica',
        difficulty:
            'Easy',
        tags: [
            'american',
            'friends',
            'sitcoms'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0055',
        category: 'TV',
        question:
            'Which actor played Gene Hunt in "Life on Mars"?',
        answers: [
            'Philip Glenister',
            'John Simm',
            'Keeley Hawes',
            'Dean Andrews'
        ],
        correctAnswer:
            'Philip Glenister',
        difficulty:
            'Medium',
        tags: [
            'british',
            'drama',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0056',
        category: 'TV',
        question:
            'Which sitcom features the characters Mark Corrigan and Jeremy Usborne?',
        answers: [
            'Peep Show',
            'Spaced',
            'The IT Crowd',
            'Green Wing'
        ],
        correctAnswer:
            'Peep Show',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            '2000s'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0057',
        category: 'TV',
        question:
            'Which actress played DCI Jane Tennison in "Prime Suspect"?',
        answers: [
            'Helen Mirren',
            'Amanda Redman',
            'Suranne Jones',
            'Lesley Sharp'
        ],
        correctAnswer:
            'Helen Mirren',
        difficulty:
            'Medium',
        tags: [
            'british',
            'crime',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0058',
        category: 'TV',
        question:
            'Which British sitcom is set in the offices of Reynholm Industries?',
        answers: [
            'The IT Crowd',
            'The Office',
            'Green Wing',
            'Black Books'
        ],
        correctAnswer:
            'The IT Crowd',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            '2000s'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0059',
        category: 'TV',
        question:
            'Which actress played Villanelle in "Killing Eve"?',
        answers: [
            'Jodie Comer',
            'Sandra Oh',
            'Suranne Jones',
            'Keeley Hawes'
        ],
        correctAnswer:
            'Jodie Comer',
        difficulty:
            'Easy',
        tags: [
            'british',
            'drama',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0060',
        category: 'TV',
        question:
            'In "Only Fools and Horses", what is the name of Del Boy and Rodney\'s grandfather?',
        answers: [
            'Grandad',
            'Albert',
            'Freddie',
            'Reg'
        ],
        correctAnswer:
            'Grandad',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'only fools and horses'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FILM
    // =========================================================

    {
        id: 'film_0043',
        category: 'Film',
        question:
            'Which actor played Neo in "The Matrix"?',
        answers: [
            'Keanu Reeves',
            'Brad Pitt',
            'Tom Cruise',
            'Johnny Depp'
        ],
        correctAnswer:
            'Keanu Reeves',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'actors',
            'science fiction'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0044',
        category: 'Film',
        question:
            'Which 1993 film stars Robin Williams as a father disguised as an elderly housekeeper?',
        answers: [
            'Mrs. Doubtfire',
            'Hook',
            'Jumanji',
            'Patch Adams'
        ],
        correctAnswer:
            'Mrs. Doubtfire',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'comedy',
            'films'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0045',
        category: 'Film',
        question:
            'Which film features a hotel called the Overlook?',
        answers: [
            'The Shining',
            'Psycho',
            'The Exorcist',
            'Poltergeist'
        ],
        correctAnswer:
            'The Shining',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'horror',
            'films'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0046',
        category: 'Film',
        question:
            'Which actor played Captain Jack Sparrow in the "Pirates of the Caribbean" films?',
        answers: [
            'Johnny Depp',
            'Orlando Bloom',
            'Geoffrey Rush',
            'Jude Law'
        ],
        correctAnswer:
            'Johnny Depp',
        difficulty:
            'Easy',
        tags: [
            '2000s',
            'actors',
            'film series'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0047',
        category: 'Film',
        question:
            'Which 1982 film features an alien who wants to "phone home"?',
        answers: [
            'E.T. the Extra-Terrestrial',
            'Cocoon',
            'Starman',
            'The Last Starfighter'
        ],
        correctAnswer:
            'E.T. the Extra-Terrestrial',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'science fiction',
            'films'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0048',
        category: 'Film',
        question:
            'Which actress played Sarah Connor in "Terminator 2: Judgment Day"?',
        answers: [
            'Linda Hamilton',
            'Sigourney Weaver',
            'Jamie Lee Curtis',
            'Geena Davis'
        ],
        correctAnswer:
            'Linda Hamilton',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'actors',
            'action'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GENERAL KNOWLEDGE
    // =========================================================

    {
        id: 'general_0022',
        category: 'General Knowledge',
        question:
            'How many minutes are there in three hours?',
        answers: [
            '180',
            '160',
            '200',
            '240'
        ],
        correctAnswer:
            '180',
        difficulty:
            'Easy',
        tags: [
            'numbers',
            'time',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0023',
        category: 'General Knowledge',
        question:
            'Which precious stone is traditionally associated with a 40th wedding anniversary?',
        answers: [
            'Ruby',
            'Diamond',
            'Sapphire',
            'Emerald'
        ],
        correctAnswer:
            'Ruby',
        difficulty:
            'Medium',
        tags: [
            'traditions',
            'anniversaries',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0024',
        category: 'General Knowledge',
        question:
            'Which chess piece moves in an L-shape?',
        answers: [
            'Knight',
            'Bishop',
            'Rook',
            'Queen'
        ],
        correctAnswer:
            'Knight',
        difficulty:
            'Easy',
        tags: [
            'chess',
            'games',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // HISTORY
    // =========================================================

    {
        id: 'history_0021',
        category: 'History',
        question:
            'Which British monarch was on the throne when the Second World War began in 1939?',
        answers: [
            'George VI',
            'George V',
            'Edward VIII',
            'Elizabeth II'
        ],
        correctAnswer:
            'George VI',
        difficulty:
            'Medium',
        tags: [
            'british',
            'monarchy',
            'second world war'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0022',
        category: 'History',
        question:
            'Which ancient civilisation built the Colosseum?',
        answers: [
            'Romans',
            'Greeks',
            'Egyptians',
            'Vikings'
        ],
        correctAnswer:
            'Romans',
        difficulty:
            'Easy',
        tags: [
            'ancient history',
            'rome',
            'buildings'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0023',
        category: 'History',
        question:
            'Who was the first person to walk on the Moon?',
        answers: [
            'Neil Armstrong',
            'Buzz Aldrin',
            'Yuri Gagarin',
            'John Glenn'
        ],
        correctAnswer:
            'Neil Armstrong',
        difficulty:
            'Easy',
        tags: [
            'space',
            '20th century',
            'history'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GEOGRAPHY
    // =========================================================

    {
        id: 'geography_0021',
        category: 'Geography',
        question:
            'What is the capital city of Spain?',
        answers: [
            'Madrid',
            'Barcelona',
            'Seville',
            'Valencia'
        ],
        correctAnswer:
            'Madrid',
        difficulty:
            'Easy',
        tags: [
            'europe',
            'capitals',
            'spain'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0022',
        category: 'Geography',
        question:
            'Which river flows through Rome?',
        answers: [
            'Tiber',
            'Po',
            'Arno',
            'Adige'
        ],
        correctAnswer:
            'Tiber',
        difficulty:
            'Medium',
        tags: [
            'italy',
            'rivers',
            'europe'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0023',
        category: 'Geography',
        question:
            'Which European country has Ljubljana as its capital?',
        answers: [
            'Slovenia',
            'Slovakia',
            'Croatia',
            'Serbia'
        ],
        correctAnswer:
            'Slovenia',
        difficulty:
            'Medium',
        tags: [
            'europe',
            'capitals',
            'countries'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SCIENCE & NATURE
    // =========================================================

    {
        id: 'science_0020',
        category: 'Science & Nature',
        question:
            'Which gas do humans need to breathe in to survive?',
        answers: [
            'Oxygen',
            'Nitrogen',
            'Carbon dioxide',
            'Helium'
        ],
        correctAnswer:
            'Oxygen',
        difficulty:
            'Easy',
        tags: [
            'biology',
            'human body',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0021',
        category: 'Science & Nature',
        question:
            'Which planet is closest in size to Earth?',
        answers: [
            'Venus',
            'Mars',
            'Mercury',
            'Neptune'
        ],
        correctAnswer:
            'Venus',
        difficulty:
            'Medium',
        tags: [
            'space',
            'planets',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0022',
        category: 'Science & Nature',
        question:
            'What force pulls objects towards the centre of the Earth?',
        answers: [
            'Gravity',
            'Magnetism',
            'Friction',
            'Electricity'
        ],
        correctAnswer:
            'Gravity',
        difficulty:
            'Easy',
        tags: [
            'physics',
            'forces',
            'science'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FOOD & DRINK
    // =========================================================

    {
        id: 'food_0014',
        category: 'Food & Drink',
        question:
            'Which herb is traditionally one of the main ingredients in pesto?',
        answers: [
            'Basil',
            'Parsley',
            'Rosemary',
            'Sage'
        ],
        correctAnswer:
            'Basil',
        difficulty:
            'Easy',
        tags: [
            'italian',
            'ingredients',
            'food'
        ],
        dailyEligible:
            true
    },

    {
        id: 'food_0015',
        category: 'Food & Drink',
        question:
            'Which Italian dish is made with layers of pasta, sauce and typically cheese?',
        answers: [
            'Lasagne',
            'Risotto',
            'Gnocchi',
            'Focaccia'
        ],
        correctAnswer:
            'Lasagne',
        difficulty:
            'Easy',
        tags: [
            'italian',
            'food',
            'pasta'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SPORT
    // =========================================================

    {
        id: 'sport_0014',
        category: 'Sport',
        question:
            'How many holes are played in a standard round of golf?',
        answers: [
            '18',
            '16',
            '20',
            '24'
        ],
        correctAnswer:
            '18',
        difficulty:
            'Easy',
        tags: [
            'golf',
            'rules',
            'sport'
        ],
        dailyEligible:
            true
    },

    {
        id: 'sport_0015',
        category: 'Sport',
        question:
            'In snooker, which coloured ball is worth six points?',
        answers: [
            'Pink',
            'Blue',
            'Brown',
            'Green'
        ],
        correctAnswer:
            'Pink',
        difficulty:
            'Medium',
        tags: [
            'snooker',
            'scoring',
            'sport'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // LITERATURE
    // =========================================================

    {
        id: 'literature_0012',
        category: 'Literature',
        question:
            'Who wrote "The Hobbit"?',
        answers: [
            'J. R. R. Tolkien',
            'C. S. Lewis',
            'George Orwell',
            'H. G. Wells'
        ],
        correctAnswer:
            'J. R. R. Tolkien',
        difficulty:
            'Easy',
        tags: [
            'british',
            'authors',
            'fantasy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'literature_0013',
        category: 'Literature',
        question:
            'Which novel by Mary Shelley features Victor Frankenstein?',
        answers: [
            'Frankenstein',
            'Dracula',
            'The Woman in White',
            'Wuthering Heights'
        ],
        correctAnswer:
            'Frankenstein',
        difficulty:
            'Easy',
        tags: [
            'british',
            'classic literature',
            'novels'
        ],
        dailyEligible:
            true
    }

];

module.exports = questions;
