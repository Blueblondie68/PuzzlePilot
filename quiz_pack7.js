// quiz_pack7.js
// PuzzlePilot Quiz - Question Pack 7
// 40 additional questions

const questions = [

    // =========================================================
    // MUSIC
    // =========================================================

    {
        id: 'music_0045',
        category: 'Music',
        question:
            'Which singer released the 1983 hit "Total Eclipse of the Heart"?',
        answers: [
            'Bonnie Tyler',
            'Kim Wilde',
            'Belinda Carlisle',
            'Pat Benatar'
        ],
        correctAnswer:
            'Bonnie Tyler',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'british',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0046',
        category: 'Music',
        question:
            'Which band released the album "The Joshua Tree" in 1987?',
        answers: [
            'U2',
            'Simple Minds',
            'INXS',
            'R.E.M.'
        ],
        correctAnswer:
            'U2',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'albums',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0047',
        category: 'Music',
        question:
            'Which band released the 1985 hit "Take On Me"?',
        answers: [
            'A-ha',
            'Simple Minds',
            'Ultravox',
            'Talk Talk'
        ],
        correctAnswer:
            'A-ha',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'bands',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0048',
        category: 'Music',
        question:
            'Which group recorded the 1999 hit "No Scrubs"?',
        answers: [
            'TLC',
            'Destiny\'s Child',
            'En Vogue',
            'All Saints'
        ],
        correctAnswer:
            'TLC',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'groups',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0049',
        category: 'Music',
        question:
            'Which singer was the lead vocalist of The Jam?',
        answers: [
            'Paul Weller',
            'Joe Strummer',
            'Ian Dury',
            'Paul Young'
        ],
        correctAnswer:
            'Paul Weller',
        difficulty:
            'Medium',
        tags: [
            'british',
            'bands',
            '1970s'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0050',
        category: 'Music',
        question:
            'Which duo released the 1981 hit "Tainted Love"?',
        answers: [
            'Soft Cell',
            'Erasure',
            'Pet Shop Boys',
            'Yazoo'
        ],
        correctAnswer:
            'Soft Cell',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'duos'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0051',
        category: 'Music',
        question:
            'Which singer released the album "Whitney" in 1987?',
        answers: [
            'Whitney Houston',
            'Diana Ross',
            'Anita Baker',
            'Janet Jackson'
        ],
        correctAnswer:
            'Whitney Houston',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'albums',
            'singers'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0052',
        category: 'Music',
        question:
            'Which British band had a 1997 hit with "Bitter Sweet Symphony"?',
        answers: [
            'The Verve',
            'Blur',
            'Pulp',
            'Suede'
        ],
        correctAnswer:
            'The Verve',
        difficulty:
            'Medium',
        tags: [
            '1990s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // TELEVISION
    // =========================================================

    {
        id: 'tv_0045',
        category: 'TV',
        question:
            'Which actor played Victor Meldrew in "One Foot in the Grave"?',
        answers: [
            'Richard Wilson',
            'David Jason',
            'Geoffrey Palmer',
            'Richard Briers'
        ],
        correctAnswer:
            'Richard Wilson',
        difficulty:
            'Easy',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0046',
        category: 'TV',
        question:
            'In "The Simpsons", what is the name of Homer Simpson\'s next-door neighbour?',
        answers: [
            'Ned Flanders',
            'Barney Gumble',
            'Seymour Skinner',
            'Moe Szyslak'
        ],
        correctAnswer:
            'Ned Flanders',
        difficulty:
            'Easy',
        tags: [
            'american',
            'animation',
            'the simpsons'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0047',
        category: 'TV',
        question:
            'Which actress played Sybil Fawlty in "Fawlty Towers"?',
        answers: [
            'Prunella Scales',
            'Penelope Keith',
            'Felicity Kendal',
            'Wendy Craig'
        ],
        correctAnswer:
            'Prunella Scales',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0048',
        category: 'TV',
        question:
            'What is the surname of the family at the centre of "The Royle Family"?',
        answers: [
            'Royle',
            'Harper',
            'Boswell',
            'Garvey'
        ],
        correctAnswer:
            'Royle',
        difficulty:
            'Easy',
        tags: [
            'british',
            'sitcoms',
            '1990s'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0049',
        category: 'TV',
        question:
            'Which actor played Sherlock Holmes in the BBC series "Sherlock"?',
        answers: [
            'Benedict Cumberbatch',
            'Martin Freeman',
            'Andrew Scott',
            'Tom Hiddleston'
        ],
        correctAnswer:
            'Benedict Cumberbatch',
        difficulty:
            'Easy',
        tags: [
            'british',
            'drama',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0050',
        category: 'TV',
        question:
            'In "Dad\'s Army", what is Captain Mainwaring\'s civilian job?',
        answers: [
            'Bank manager',
            'Butcher',
            'Undertaker',
            'Solicitor'
        ],
        correctAnswer:
            'Bank manager',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'classic television'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0051',
        category: 'TV',
        question:
            'Which actor played Tony Soprano in "The Sopranos"?',
        answers: [
            'James Gandolfini',
            'Steve Buscemi',
            'Michael Imperioli',
            'Joe Pantoliano'
        ],
        correctAnswer:
            'James Gandolfini',
        difficulty:
            'Medium',
        tags: [
            'american',
            'drama',
            '1990s',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0052',
        category: 'TV',
        question:
            'Which British comedy series featured the characters Tim, Dawn, Gareth and David Brent?',
        answers: [
            'The Office',
            'Extras',
            'Spaced',
            'Green Wing'
        ],
        correctAnswer:
            'The Office',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            '2000s'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FILM
    // =========================================================

    {
        id: 'film_0037',
        category: 'Film',
        question:
            'Which actor played Rocky Balboa in the "Rocky" films?',
        answers: [
            'Sylvester Stallone',
            'Arnold Schwarzenegger',
            'Bruce Willis',
            'Jean-Claude Van Damme'
        ],
        correctAnswer:
            'Sylvester Stallone',
        difficulty:
            'Easy',
        tags: [
            'actors',
            '1970s',
            'film series'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0038',
        category: 'Film',
        question:
            'Which 1990 film stars Macaulay Culkin as Kevin McCallister?',
        answers: [
            'Home Alone',
            'Uncle Buck',
            'Richie Rich',
            'My Girl'
        ],
        correctAnswer:
            'Home Alone',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'comedy',
            'films'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0039',
        category: 'Film',
        question:
            'Which actress played Vivian Ward in "Pretty Woman"?',
        answers: [
            'Julia Roberts',
            'Sandra Bullock',
            'Meg Ryan',
            'Demi Moore'
        ],
        correctAnswer:
            'Julia Roberts',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'actors',
            'romance'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0040',
        category: 'Film',
        question:
            'Which film features the line "Nobody puts Baby in a corner"?',
        answers: [
            'Dirty Dancing',
            'Flashdance',
            'Footloose',
            'Fame'
        ],
        correctAnswer:
            'Dirty Dancing',
        difficulty:
            'Easy',
        tags: [
            '1980s',
            'films',
            'romance'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0041',
        category: 'Film',
        question:
            'Which actor played the eccentric inventor Willy Wonka in the 1971 film "Willy Wonka & the Chocolate Factory"?',
        answers: [
            'Gene Wilder',
            'Peter Sellers',
            'Dudley Moore',
            'Richard Pryor'
        ],
        correctAnswer:
            'Gene Wilder',
        difficulty:
            'Medium',
        tags: [
            '1970s',
            'actors',
            'films'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0042',
        category: 'Film',
        question:
            'Which actress played the title character in the 2001 film "Amélie"?',
        answers: [
            'Audrey Tautou',
            'Juliette Binoche',
            'Marion Cotillard',
            'Sophie Marceau'
        ],
        correctAnswer:
            'Audrey Tautou',
        difficulty:
            'Medium',
        tags: [
            '2000s',
            'actors',
            'french film'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GENERAL KNOWLEDGE
    // =========================================================

    {
        id: 'general_0019',
        category: 'General Knowledge',
        question:
            'How many sides does an octagon have?',
        answers: [
            '8',
            '6',
            '10',
            '12'
        ],
        correctAnswer:
            '8',
        difficulty:
            'Easy',
        tags: [
            'shapes',
            'numbers',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0020',
        category: 'General Knowledge',
        question:
            'What colour are the stars on the flag of the European Union?',
        answers: [
            'Gold',
            'White',
            'Red',
            'Silver'
        ],
        correctAnswer:
            'Gold',
        difficulty:
            'Easy',
        tags: [
            'flags',
            'europe',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0021',
        category: 'General Knowledge',
        question:
            'In a standard pack of playing cards, how many queens are there?',
        answers: [
            '4',
            '2',
            '6',
            '8'
        ],
        correctAnswer:
            '4',
        difficulty:
            'Easy',
        tags: [
            'cards',
            'numbers',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // HISTORY
    // =========================================================

    {
        id: 'history_0018',
        category: 'History',
        question:
            'Which famous London building was badly damaged by fire in 1834?',
        answers: [
            'The Palace of Westminster',
            'Buckingham Palace',
            'St Paul\'s Cathedral',
            'The Tower of London'
        ],
        correctAnswer:
            'The Palace of Westminster',
        difficulty:
            'Medium',
        tags: [
            'british',
            '19th century',
            'london'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0019',
        category: 'History',
        question:
            'Which Roman city was destroyed alongside Pompeii when Mount Vesuvius erupted in AD 79?',
        answers: [
            'Herculaneum',
            'Sparta',
            'Carthage',
            'Corinth'
        ],
        correctAnswer:
            'Herculaneum',
        difficulty:
            'Medium',
        tags: [
            'ancient history',
            'rome',
            'vesuvius'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0020',
        category: 'History',
        question:
            'Which monarch was on the British throne for most of the Victorian era?',
        answers: [
            'Queen Victoria',
            'George III',
            'Edward VII',
            'William IV'
        ],
        correctAnswer:
            'Queen Victoria',
        difficulty:
            'Easy',
        tags: [
            'british',
            'monarchy',
            '19th century'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GEOGRAPHY
    // =========================================================

    {
        id: 'geography_0018',
        category: 'Geography',
        question:
            'Which river flows through Paris?',
        answers: [
            'Seine',
            'Rhine',
            'Danube',
            'Loire'
        ],
        correctAnswer:
            'Seine',
        difficulty:
            'Easy',
        tags: [
            'france',
            'rivers',
            'europe'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0019',
        category: 'Geography',
        question:
            'What is the capital city of Norway?',
        answers: [
            'Oslo',
            'Stockholm',
            'Copenhagen',
            'Helsinki'
        ],
        correctAnswer:
            'Oslo',
        difficulty:
            'Easy',
        tags: [
            'europe',
            'capitals',
            'norway'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0020',
        category: 'Geography',
        question:
            'Which African country contains the city of Casablanca?',
        answers: [
            'Morocco',
            'Tunisia',
            'Algeria',
            'Egypt'
        ],
        correctAnswer:
            'Morocco',
        difficulty:
            'Easy',
        tags: [
            'africa',
            'cities',
            'countries'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SCIENCE & NATURE
    // =========================================================

    {
        id: 'science_0017',
        category: 'Science & Nature',
        question:
            'Which gas makes up the largest proportion of Earth\'s atmosphere?',
        answers: [
            'Nitrogen',
            'Oxygen',
            'Carbon dioxide',
            'Hydrogen'
        ],
        correctAnswer:
            'Nitrogen',
        difficulty:
            'Easy',
        tags: [
            'earth',
            'atmosphere',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0018',
        category: 'Science & Nature',
        question:
            'What is the chemical symbol for iron?',
        answers: [
            'Fe',
            'Ir',
            'In',
            'I'
        ],
        correctAnswer:
            'Fe',
        difficulty:
            'Medium',
        tags: [
            'chemistry',
            'elements',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0019',
        category: 'Science & Nature',
        question:
            'Which planet is famous for its prominent ring system?',
        answers: [
            'Saturn',
            'Mars',
            'Venus',
            'Mercury'
        ],
        correctAnswer:
            'Saturn',
        difficulty:
            'Easy',
        tags: [
            'space',
            'planets',
            'science'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FOOD & DRINK
    // =========================================================

    {
        id: 'food_0012',
        category: 'Food & Drink',
        question:
            'Which fruit is the main ingredient in guacamole?',
        answers: [
            'Avocado',
            'Lime',
            'Tomato',
            'Mango'
        ],
        correctAnswer:
            'Avocado',
        difficulty:
            'Easy',
        tags: [
            'food',
            'ingredients',
            'mexican'
        ],
        dailyEligible:
            true
    },

    {
        id: 'food_0013',
        category: 'Food & Drink',
        question:
            'What type of food is Arborio?',
        answers: [
            'Rice',
            'Pasta',
            'Cheese',
            'Bread'
        ],
        correctAnswer:
            'Rice',
        difficulty:
            'Medium',
        tags: [
            'italian',
            'ingredients',
            'food'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SPORT
    // =========================================================

    {
        id: 'sport_0012',
        category: 'Sport',
        question:
            'How many players from one team are on the court at a time in basketball?',
        answers: [
            '5',
            '6',
            '7',
            '8'
        ],
        correctAnswer:
            '5',
        difficulty:
            'Easy',
        tags: [
            'basketball',
            'rules',
            'sport'
        ],
        dailyEligible:
            true
    },

    {
        id: 'sport_0013',
        category: 'Sport',
        question:
            'In cricket, how many runs are scored when the batter hits the ball over the boundary without it bouncing?',
        answers: [
            '6',
            '4',
            '5',
            '8'
        ],
        correctAnswer:
            '6',
        difficulty:
            'Easy',
        tags: [
            'cricket',
            'scoring',
            'sport'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // LITERATURE
    // =========================================================

    {
        id: 'literature_0010',
        category: 'Literature',
        question:
            'Who wrote "Charlie and the Chocolate Factory"?',
        answers: [
            'Roald Dahl',
            'Michael Morpurgo',
            'David Walliams',
            'E. B. White'
        ],
        correctAnswer:
            'Roald Dahl',
        difficulty:
            'Easy',
        tags: [
            'british',
            'authors',
            'children\'s literature'
        ],
        dailyEligible:
            true
    },

    {
        id: 'literature_0011',
        category: 'Literature',
        question:
            'Which Charles Dickens novel features the character Miss Havisham?',
        answers: [
            'Great Expectations',
            'Oliver Twist',
            'Bleak House',
            'David Copperfield'
        ],
        correctAnswer:
            'Great Expectations',
        difficulty:
            'Medium',
        tags: [
            'british',
            'charles dickens',
            'novels'
        ],
        dailyEligible:
            true
    }

];

module.exports = questions;
