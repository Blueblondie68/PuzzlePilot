// quiz_pack14.js
// PuzzlePilot Quiz - Question Pack 14
// 40 additional questions

const questions = [

    // =========================================================
    // MUSIC
    // =========================================================

    {
        id: 'music_0101',
        category: 'Music',
        question:
            'Which band released the 1987 hit "Never Let Me Down Again"?',
        answers: [
            'Depeche Mode',
            'New Order',
            'The Cure',
            'Pet Shop Boys'
        ],
        correctAnswer:
            'Depeche Mode',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0102',
        category: 'Music',
        question:
            'Which band had a 1980 hit with "Turning Japanese"?',
        answers: [
            'The Vapors',
            'The Jam',
            'The Undertones',
            'XTC'
        ],
        correctAnswer:
            'The Vapors',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0103',
        category: 'Music',
        question:
            'Which British band had a 1972 hit with "All the Young Dudes"?',
        answers: [
            'Mott the Hoople',
            'T. Rex',
            'Slade',
            'Sweet'
        ],
        correctAnswer:
            'Mott the Hoople',
        difficulty:
            'Medium',
        tags: [
            '1970s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0104',
        category: 'Music',
        question:
            'Which singer released the album "No Angel", featuring the hit "Thank You"?',
        answers: [
            'Dido',
            'Sophie Ellis-Bextor',
            'Natalie Imbruglia',
            'Nelly Furtado'
        ],
        correctAnswer:
            'Dido',
        difficulty:
            'Medium',
        tags: [
            '2000s',
            'british',
            'albums'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0105',
        category: 'Music',
        question:
            'Which band recorded the 1966 song "Sunny Afternoon"?',
        answers: [
            'The Kinks',
            'The Who',
            'The Hollies',
            'The Animals'
        ],
        correctAnswer:
            'The Kinks',
        difficulty:
            'Medium',
        tags: [
            '1960s',
            'british',
            'bands'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0106',
        category: 'Music',
        question:
            'Which singer released the 1986 hit "True Colors"?',
        answers: [
            'Cyndi Lauper',
            'Belinda Carlisle',
            'Kim Wilde',
            'Pat Benatar'
        ],
        correctAnswer:
            'Cyndi Lauper',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'singers',
            'songs'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0107',
        category: 'Music',
        question:
            'Which British band released the 1995 album "(What\'s the Story) Morning Glory?"?',
        answers: [
            'Oasis',
            'Blur',
            'Pulp',
            'Suede'
        ],
        correctAnswer:
            'Oasis',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'british',
            'albums'
        ],
        dailyEligible:
            true
    },

    {
        id: 'music_0108',
        category: 'Music',
        question:
            'Which singer released the 2006 hit "Smile"?',
        answers: [
            'Lily Allen',
            'Amy Winehouse',
            'Kate Nash',
            'Duffy'
        ],
        correctAnswer:
            'Lily Allen',
        difficulty:
            'Medium',
        tags: [
            '2000s',
            'british',
            'singers'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // TELEVISION
    // =========================================================

    {
        id: 'tv_0101',
        category: 'Television',
        question:
            'In "Red Dwarf", what is the surname of the character Dave?',
        answers: [
            'Lister',
            'Rimmer',
            'Kryten',
            'Cat'
        ],
        correctAnswer:
            'Lister',
        difficulty:
            'Easy',
        tags: [
            'british',
            'comedy',
            'red dwarf'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0102',
        category: 'Television',
        question:
            'Which actress played Buffy Summers in "Buffy the Vampire Slayer"?',
        answers: [
            'Sarah Michelle Gellar',
            'Alyson Hannigan',
            'Charisma Carpenter',
            'Eliza Dushku'
        ],
        correctAnswer:
            'Sarah Michelle Gellar',
        difficulty:
            'Easy',
        tags: [
            'american',
            '1990s',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0103',
        category: 'Television',
        question:
            'Which actress played DCI Cassie Stuart in "Unforgotten"?',
        answers: [
            'Nicola Walker',
            'Suranne Jones',
            'Sarah Lancashire',
            'Keeley Hawes'
        ],
        correctAnswer:
            'Nicola Walker',
        difficulty:
            'Medium',
        tags: [
            'british',
            'crime',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0104',
        category: 'Television',
        question:
            'Which actor played Bernard Black in the sitcom "Black Books"?',
        answers: [
            'Dylan Moran',
            'Bill Bailey',
            'Simon Pegg',
            'Chris O\'Dowd'
        ],
        correctAnswer:
            'Dylan Moran',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0105',
        category: 'Television',
        question:
            'Which actress played Anne Lister in the television drama "Gentleman Jack"?',
        answers: [
            'Suranne Jones',
            'Sarah Lancashire',
            'Nicola Walker',
            'Maxine Peake'
        ],
        correctAnswer:
            'Suranne Jones',
        difficulty:
            'Medium',
        tags: [
            'british',
            'drama',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0106',
        category: 'Television',
        question:
            'Which actor played David Brent\'s colleague Gareth Keenan in "The Office"?',
        answers: [
            'Mackenzie Crook',
            'Martin Freeman',
            'Ricky Gervais',
            'Stephen Merchant'
        ],
        correctAnswer:
            'Mackenzie Crook',
        difficulty:
            'Medium',
        tags: [
            'british',
            'sitcoms',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0107',
        category: 'Television',
        question:
            'Which actress played Detective Sergeant Ellie Miller in "Broadchurch"?',
        answers: [
            'Olivia Colman',
            'Jodie Whittaker',
            'Sarah Parish',
            'Julie Hesmondhalgh'
        ],
        correctAnswer:
            'Olivia Colman',
        difficulty:
            'Medium',
        tags: [
            'british',
            'crime',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'tv_0108',
        category: 'Television',
        question:
            'Which actor played Detective Inspector Alec Hardy in "Broadchurch"?',
        answers: [
            'David Tennant',
            'Martin Freeman',
            'John Simm',
            'James Nesbitt'
        ],
        correctAnswer:
            'David Tennant',
        difficulty:
            'Medium',
        tags: [
            'british',
            'crime',
            'actors'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FILM
    // =========================================================

    {
        id: 'film_0079',
        category: 'Film',
        question:
            'Which actor played Del Griffith in the 1987 film "Planes, Trains and Automobiles"?',
        answers: [
            'John Candy',
            'Steve Martin',
            'Dan Aykroyd',
            'Chevy Chase'
        ],
        correctAnswer:
            'John Candy',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'actors',
            'comedy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0080',
        category: 'Film',
        question:
            'Which 1997 film follows a group of unemployed Sheffield steelworkers who form a striptease act?',
        answers: [
            'The Full Monty',
            'Brassed Off',
            'Billy Elliot',
            'Calendar Girls'
        ],
        correctAnswer:
            'The Full Monty',
        difficulty:
            'Easy',
        tags: [
            '1990s',
            'british',
            'comedy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0081',
        category: 'Film',
        question:
            'Which actor played the title role in the 1982 film "Gandhi"?',
        answers: [
            'Ben Kingsley',
            'Alec Guinness',
            'Richard Attenborough',
            'John Hurt'
        ],
        correctAnswer:
            'Ben Kingsley',
        difficulty:
            'Medium',
        tags: [
            '1980s',
            'actors',
            'biographical films'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0082',
        category: 'Film',
        question:
            'Which actor played Billy Elliot\'s father Jackie in the 2000 film "Billy Elliot"?',
        answers: [
            'Gary Lewis',
            'Jim Broadbent',
            'Robert Carlyle',
            'Brendan Gleeson'
        ],
        correctAnswer:
            'Gary Lewis',
        difficulty:
            'Hard',
        tags: [
            '2000s',
            'british',
            'actors'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0083',
        category: 'Film',
        question:
            'Which actor played George Bailey in the 1946 film "It\'s a Wonderful Life"?',
        answers: [
            'James Stewart',
            'Cary Grant',
            'Gregory Peck',
            'Henry Fonda'
        ],
        correctAnswer:
            'James Stewart',
        difficulty:
            'Medium',
        tags: [
            '1940s',
            'actors',
            'classics'
        ],
        dailyEligible:
            true
    },

    {
        id: 'film_0084',
        category: 'Film',
        question:
            'Which actress played the lead role of Erin Brockovich in the 2000 film?',
        answers: [
            'Julia Roberts',
            'Sandra Bullock',
            'Nicole Kidman',
            'Meg Ryan'
        ],
        correctAnswer:
            'Julia Roberts',
        difficulty:
            'Easy',
        tags: [
            '2000s',
            'actors',
            'drama'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GENERAL KNOWLEDGE
    // =========================================================

    {
        id: 'general_0040',
        category: 'General Knowledge',
        question:
            'How many sides does a decagon have?',
        answers: [
            '10',
            '8',
            '9',
            '12'
        ],
        correctAnswer:
            '10',
        difficulty:
            'Easy',
        tags: [
            'shapes',
            'numbers',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0041',
        category: 'General Knowledge',
        question:
            'What is a baby goat called?',
        answers: [
            'Kid',
            'Calf',
            'Foal',
            'Cub'
        ],
        correctAnswer:
            'Kid',
        difficulty:
            'Easy',
        tags: [
            'animals',
            'words',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    {
        id: 'general_0042',
        category: 'General Knowledge',
        question:
            'Which country uses the forint as its currency?',
        answers: [
            'Hungary',
            'Poland',
            'Romania',
            'Croatia'
        ],
        correctAnswer:
            'Hungary',
        difficulty:
            'Medium',
        tags: [
            'currency',
            'europe',
            'general knowledge'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // HISTORY
    // =========================================================

    {
        id: 'history_0039',
        category: 'History',
        question:
            'Which English king had a flagship called the Mary Rose?',
        answers: [
            'Henry VIII',
            'Henry VII',
            'Edward VI',
            'Richard III'
        ],
        correctAnswer:
            'Henry VIII',
        difficulty:
            'Medium',
        tags: [
            'british',
            'tudors',
            'monarchy'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0040',
        category: 'History',
        question:
            'Which warrior queen led a revolt against Roman rule in Britain?',
        answers: [
            'Boudica',
            'Cleopatra',
            'Zenobia',
            'Cartimandua'
        ],
        correctAnswer:
            'Boudica',
        difficulty:
            'Easy',
        tags: [
            'british',
            'roman',
            'ancient history'
        ],
        dailyEligible:
            true
    },

    {
        id: 'history_0041',
        category: 'History',
        question:
            'Which passenger liner was sunk by a German U-boat in 1915?',
        answers: [
            'Lusitania',
            'Britannic',
            'Queen Mary',
            'Mauretania'
        ],
        correctAnswer:
            'Lusitania',
        difficulty:
            'Medium',
        tags: [
            '20th century',
            'first world war',
            'ships'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // GEOGRAPHY
    // =========================================================

    {
        id: 'geography_0039',
        category: 'Geography',
        question:
            'What is the capital city of Denmark?',
        answers: [
            'Copenhagen',
            'Oslo',
            'Stockholm',
            'Helsinki'
        ],
        correctAnswer:
            'Copenhagen',
        difficulty:
            'Easy',
        tags: [
            'europe',
            'capitals',
            'denmark'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0040',
        category: 'Geography',
        question:
            'Which African country is home to the city of Marrakech?',
        answers: [
            'Morocco',
            'Tunisia',
            'Algeria',
            'Egypt'
        ],
        correctAnswer:
            'Morocco',
        difficulty:
            'Easy',
        tags: [
            'africa',
            'countries',
            'cities'
        ],
        dailyEligible:
            true
    },

    {
        id: 'geography_0041',
        category: 'Geography',
        question:
            'Which European country has the city of Porto?',
        answers: [
            'Portugal',
            'Spain',
            'Italy',
            'Greece'
        ],
        correctAnswer:
            'Portugal',
        difficulty:
            'Easy',
        tags: [
            'europe',
            'countries',
            'cities'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SCIENCE
    // =========================================================

    {
        id: 'science_0038',
        category: 'Science',
        question:
            'Which organ in the human body filters waste from the blood to produce urine?',
        answers: [
            'Kidneys',
            'Liver',
            'Lungs',
            'Pancreas'
        ],
        correctAnswer:
            'Kidneys',
        difficulty:
            'Easy',
        tags: [
            'biology',
            'human body',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0039',
        category: 'Science',
        question:
            'Which scientist developed the first successful smallpox vaccine?',
        answers: [
            'Edward Jenner',
            'Louis Pasteur',
            'Alexander Fleming',
            'Robert Koch'
        ],
        correctAnswer:
            'Edward Jenner',
        difficulty:
            'Medium',
        tags: [
            'medicine',
            'scientists',
            'science'
        ],
        dailyEligible:
            true
    },

    {
        id: 'science_0040',
        category: 'Science',
        question:
            'Which planet is best known for its prominent ring system?',
        answers: [
            'Saturn',
            'Jupiter',
            'Uranus',
            'Neptune'
        ],
        correctAnswer:
            'Saturn',
        difficulty:
            'Easy',
        tags: [
            'space',
            'planets',
            'science'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // FOOD AND DRINK
    // =========================================================

    {
        id: 'food_0026',
        category: 'Food and Drink',
        question:
            'Which cheese is traditionally used in a classic Caesar salad?',
        answers: [
            'Parmesan',
            'Feta',
            'Cheddar',
            'Brie'
        ],
        correctAnswer:
            'Parmesan',
        difficulty:
            'Easy',
        tags: [
            'cheese',
            'salads',
            'food'
        ],
        dailyEligible:
            true
    },

    {
        id: 'food_0027',
        category: 'Food and Drink',
        question:
            'Which fruit is traditionally used as the base of a classic lemon curd?',
        answers: [
            'Lemon',
            'Orange',
            'Lime',
            'Grapefruit'
        ],
        correctAnswer:
            'Lemon',
        difficulty:
            'Easy',
        tags: [
            'fruit',
            'british',
            'food'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // SPORT
    // =========================================================

    {
        id: 'sport_0026',
        category: 'Sport',
        question:
            'Which two countries traditionally compete for cricket\'s Ashes?',
        answers: [
            'England and Australia',
            'England and India',
            'Australia and New Zealand',
            'England and South Africa'
        ],
        correctAnswer:
            'England and Australia',
        difficulty:
            'Easy',
        tags: [
            'cricket',
            'british',
            'sport'
        ],
        dailyEligible:
            true
    },

    {
        id: 'sport_0027',
        category: 'Sport',
        question:
            'The Tour de France is primarily associated with which sport?',
        answers: [
            'Cycling',
            'Running',
            'Motor racing',
            'Rowing'
        ],
        correctAnswer:
            'Cycling',
        difficulty:
            'Easy',
        tags: [
            'cycling',
            'france',
            'sport'
        ],
        dailyEligible:
            true
    },

    // =========================================================
    // LITERATURE
    // =========================================================

    {
        id: 'literature_0024',
        category: 'Literature',
        question:
            'Who wrote the novel "Animal Farm"?',
        answers: [
            'George Orwell',
            'Aldous Huxley',
            'H. G. Wells',
            'Evelyn Waugh'
        ],
        correctAnswer:
            'George Orwell',
        difficulty:
            'Easy',
        tags: [
            'british',
            'authors',
            'novels'
        ],
        dailyEligible:
            true
    },

    {
        id: 'literature_0025',
        category: 'Literature',
        question:
            'Which fictional character lives at 221B Baker Street?',
        answers: [
            'Sherlock Holmes',
            'Hercule Poirot',
            'James Bond',
            'Philip Marlowe'
        ],
        correctAnswer:
            'Sherlock Holmes',
        difficulty:
            'Easy',
        tags: [
            'british',
            'characters',
            'crime'
        ],
        dailyEligible:
            true
    }

];

module.exports = questions;
